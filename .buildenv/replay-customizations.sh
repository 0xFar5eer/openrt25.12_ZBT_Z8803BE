...
try:
    ...
    sys.stderr.write(
        "ERR: could not locate existing zbt-z8803be device block boundaries in filogic.mk\n"
    )
    sys.exit(1)
...
# build.sh auto-sources .buildenv/local.env when present. We drop a
# small env file in the destination so each replayed worktree can use
# isolated build state without editing build.sh.
# Volume name is derived from the destination directory.
DST_VOL="${DST_VOL:-$(basename "$DST")-buildvol}"
cat > "$DST/.buildenv/local.env" <<EOF
# Auto-generated by replay-customizations.sh.
# Automatically loaded by ./.buildenv/build.sh when present, so this
# checkout uses its own Docker volume without manual sourcing.
# Example:
#     ./.buildenv/build.sh init
# If you want VOL exported in your current shell for other commands,
# you can still run:
...
done < <(cd "$SRC" && git ls-files 2>/dev/null)

if [ "$audit_missing" -gt 0 ]; then
    echo "  summary: $audit_missing tracked path(s) missing in destination"
fi
