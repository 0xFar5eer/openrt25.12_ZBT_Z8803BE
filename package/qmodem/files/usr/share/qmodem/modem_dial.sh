...
update_config()
{
    ...
    # ZBT: the netifd interface metric is the router-wide route contract
    # (wired 9/10, cellular 200). QModem brings the link up but must not
    # overwrite the network-config metric with the qmodem profile value on
    # every redial, so the persistent value wins when it is numeric.
...
flush_ip_cb()
{
    ...
    if [ "$remove_target" = "$bind_modem_config" ];then
        if [ "$network_cfg" = "$interface_name" ]; then
            # ZBT: keep the stable primary section across hangs. Deleting it
            # dropped the route metric the network config holds and let the
            # next scan recreate a different route priority; the physical
            # device binding is runtime state and is cleared while the modem
            # is down. dns/peerdns are left untouched so operator edits and
            # the values written at creation survive.
