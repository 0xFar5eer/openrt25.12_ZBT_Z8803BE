 ...
 *   security / key / selected bands. netifd represents an MLD as one
...
function applyMld(rec) {
	...
	/*
	 * The MLD itself is ONE section whose device option is the list of
	 * every selected radio; netifd turns that into a single multi-link
	 * ap-mld interface spanning all bands.
	 */
	const selectedBands = allBands.filter(band => rec.bands.has(band));
	const mldIface = slug;
	writeCommon(mldIface, selectedBands.map(band => radios[band]));
	...
	/*
	 * A band removed from an existing MLD remains available as a normal
	 * AP. Per-band PMF policy:
	 *   - 6 GHz: PMF=2 required (802.11ax spec).
	 *   - Pure SAE or OWE on any band: PMF=2.
	 *   - SAE-mixed on 2.4/5 GHz: PMF=1 (optional, keeps WPA2 clients).
	 *   - psk2 / none: no PMF.
	 */
